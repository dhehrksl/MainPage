...
- expectedResult에 "~하거나 ~한다"처럼 두 가지 이상의 가능성을 나열하는 것. 기대 결과는 반드시 확정적으로 하나만 서술할 것.
...
- 상호작용 요소(버튼/입력/폼/링크)가 부족하다고 해서 "이 텍스트가 페이지에 표시되는지" 같은 단순 존재 확인 TC로 개수를 채우는 것. 이런 텍스트 확인 TC는 페이지에 실제 상호작용 요소가 거의 없을 때 보조적으로만 소수 포함하고, 절대 전체 TC의 다수를 차지하게 하지 말 것. 개수보다 품질이 우선이므로, 위 기준을 통과하는 TC가 ${numTCs}개보다 적다면 그보다 적게 반환해도 된다.
